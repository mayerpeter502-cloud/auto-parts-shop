...
export default function FilterSidebar({ filters, onChange, availableFilters }) {
  ...
  const handlePriceChange = (index, value) => {
    const newRange = [...safeFilters.priceRange];
    newRange[index] = Number(value) || 0;
    onChange({ ...safeFilters, priceRange: newRange });
  };
  ...
  const handleCheckboxChange = (type, value) => {
    ...
    const updated = current.includes(value)
      ...
      : [...current, value];
    onChange({ ...safeFilters, [type]: updated });
  };
  ...
  const hasActiveFilters = safeFilters.brands.length > 0 || 
    safeFilters.categories.length > 0 || 
    safeFilters.inStock ||
    safeFilters.priceRange[0] > 0 || 
    safeFilters.priceRange[1] < 100000;
        ...
        {expanded.availability && (
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              onChange={(e) => onChange({ ...safeFilters, inStock: e.target.checked })}
              className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500"
            />
